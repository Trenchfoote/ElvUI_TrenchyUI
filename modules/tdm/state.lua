...
if Enum.DamageMeterType.EnemyDamageTaken then TDM.MODE_SHORT[Enum.DamageMeterType.EnemyDamageTaken] = "Enemy Taken" end
...
-- Exact death time from C_DamageMeter; SecretWhenInCombat for addons so it's
-- nil during the live pull and the precise value once combat ends (no estimate).
local mfloor = math.floor

function TDM.GetDeathTimeText(src)
    local dt = src.deathTimeSeconds
    if dt and E:NotSecretValue(dt) and dt > 0 then
        return format('%d:%02d', mfloor(dt / 60), mfloor(dt % 60))
    end
end
