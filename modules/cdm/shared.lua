...
-- Spell cooldown override: replace buff duration with spell CD on essential/utility icons
local function OverrideCooldownWithSpellCD(cooldown)
	if cooldown._tuiInOverride then return end
	local itemFrame = cooldown:GetParent()
	local sid = itemFrame and itemFrame.GetBaseSpellID and itemFrame:GetBaseSpellID()
	if not sid then return end
	local dur = C_Spell.GetSpellCooldownDuration(sid)
	if not dur then return end
	cooldown._tuiInOverride = true
	cooldown:SetCooldownFromDurationObject(dur)
	cooldown._tuiInOverride = nil
end

function S.HookSpellCooldownOverride(itemFrame)
	local cooldown = itemFrame.Cooldown
	if not cooldown or cooldown._tuiSpellCDHooked then return end
	cooldown._tuiSpellCDHooked = true
	hooksecurefunc(cooldown, 'SetCooldown', function(self)
		OverrideCooldownWithSpellCD(self)
	end)
	hooksecurefunc(cooldown, 'SetCooldownFromDurationObject', function(self)
		OverrideCooldownWithSpellCD(self)
	end)
	-- Apply immediately for icons that already have a cooldown running
	OverrideCooldownWithSpellCD(cooldown)
end

function S.ApplySpellCooldownDesaturation(itemFrame)
	local icon = itemFrame.Icon
	if not icon then return end
	local sid = itemFrame.GetBaseSpellID and itemFrame:GetBaseSpellID()
	if not sid then return end

	-- Proc active: spell is usable despite CD, don't desaturate
	local alert = itemFrame.SpellActivationAlert
	if alert and alert:IsShown() then
	...
	-- Charge spells: only desaturate when all charges are spent
	...
	if charges and charges.maxCharges > 1 then
		...
		if not issecretvalue(cur) then
			icon:SetDesaturated(cur == 0)
		else
			-- In combat with secret charges: fall back to recharge isActive
			icon:SetDesaturated(charges.isActive)
		end
		return
	end

	icon:SetDesaturated(info.isActive)
end
